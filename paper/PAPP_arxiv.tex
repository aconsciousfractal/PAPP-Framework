...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{../paper_build/figures/Elements/PAPP polychora.png}
\caption{Examples of 4D polychora generated by PAPP Stage 4 construction. Four representative structures with different vertex counts demonstrate the diversity of geometries produced by the formula $V = a + 2b + 2c + d$ across parameter space. Each polychoron represents a point cloud distribution on $S^3$ before Hopf projection to $S^2$.}
\label{fig:papp_polychora}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{../paper_build/figures/Elements/Papp crystals.png}
\caption{Final 3D spherical triangulations after complete PAPP pipeline (Stages 1--6). Left panels show classical rendering; right panels display quantum metric visualization with edge tension mapping. Top row: V=18 ground state attractor (15\% occurrence frequency). Bottom row: V=59591 high-complexity structure demonstrating scalability to large vertex counts. Both structures exhibit multi-component topology with disconnected mesh regions characteristic of PAPP-generated geometries.}
\label{fig:papp_crystals}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{../paper_build/figures/Elements/Polychora classic.png}
\caption{The six convex regular 4-polytopes used for validation (Coxeter classification \cite{coxeter1973regular}): 5-cell \{3,3,3\}, 8-cell \{4,3,3\}, 16-cell \{3,3,4\}, 24-cell \{3,4,3\}, 120-cell \{5,3,3\}, and 600-cell \{3,3,5\}. PAPP successfully reconstructs all vertex counts, edge counts, and edge lengths from Coxeter group coordinates, providing ground truth validation for Hopf projection fidelity (Table~\ref{tab:pantheon}).}
\label{fig:polychora_classic}
\end{figure}
...
\item \textbf{Open-source code}: Complete implementation in Appendix A. Public repository: \url{https://github.com/aconsciousfractal/PAPP-Framework}
\item \textbf{Public dataset}: 600 MB topology database (public DOI upon publication)
...
\textbf{Why the difference?} The Hopf projection maps 4D points to $S^2$, but only spatially cohesive points are included in the convex hull triangulation. Outlier points project to isolated locations and form singleton components in the connectivity graph (see Section 3.2). The convex hull vertex count $V$ is the topologically significant parameter, as it determines the mesh structure satisfying $\chi=2$.
